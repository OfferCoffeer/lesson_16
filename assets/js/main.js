...
let getLocalDate = (date, isSeconds = false, isISO = false) => {
  ...
  result = isSeconds ? date.toLocaleString() : date.toLocaleString().replace(noSec, '');

  if (isISO) result = isSeconds ? `${year}-${month}-${day}, ${hour}:${minutes}:${seconds}` : `${year}-${month}-${day}, ${hour}:${minutes}`;
